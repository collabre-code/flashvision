...
#function for sending email alert
def email_alert(subject,body,to):
    ...
    user = "therin.young@gmail.com"
    msg['from'] = user
    password = "Gagiant1.229"
...
for cam_location in location_lst:
    ...
    if cam_location == location_lst[0]:
        ...
        location.append(rows[1])
        locationtime.append(rows[5])
        timeofday.append(rows[6])
...
for i in range(0,result_cleaned.shape[0]):
    if result_cleaned['timeDelta'][i] > 45.0:
        
        body = "%s is not active. Please visit vision.flashparking.com to troubleshoot." % (result_cleaned['Cam_ID'][i])
     
        email_alert("Flash Cam Status",body,"therin.young@gmail.com")
    else:
        continue
